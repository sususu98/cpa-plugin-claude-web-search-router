...
- ModelRouter + executor plugin (`claude-web-search-router`)
- Detects Claude / Anthropic requests with `web_search_20250305` or `web_search_20260209`
- Routes `fallback` chain: antigravity → codex → xai → tavily
- Tavily path synthesizes Claude `server_tool_use` / `web_search_tool_result` SSE or JSON

## Build

```bash
go test ./...
CGO_ENABLED=1 go build -buildmode=c-shared -o dist/claude-web-search-router.dylib ./cmd/claude-web-search-router
```
...
plugins:
  path:
    - /absolute/path/to/dist/claude-web-search-router.dylib
  configs:
...
Requires CPA host **v7.2.31+** (`github.com/router-for-me/CLIProxyAPI/v7`).
